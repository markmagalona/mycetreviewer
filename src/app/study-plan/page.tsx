...
// AI-generated 30-day personalized study plan
// Based on: diagnostic weak topics, target exam, days until exam
...
export default function StudyPlanPage() {
  ...
  useEffect(() => {
    ...
    // Load weak topics from last diagnostic
  ...
  async function generatePlan() {
    ...
    try {
      ...
      if (data.studyPlan) {
        ...
        setPlan(generateRuleBasedPlan(weakTopics, targetExam, planDays))
        setGenerated(true)
      }
    } catch {
      // Always fallback to rule-based plan
  ...
  function generateRuleBasedPlan(topics: string[], exam: string, days: number): WeekPlan[] {
    ...
    for (let w = 0; w < Math.min(totalWeeks, 4); w++) {
      const isLastWeek = w === totalWeeks - 1 || w === 3
      ...
      for (let d = 0; d < 7; d++) {
        ...
        if (d === 6) {
          // Rest day
          weekDays.push({ day: dayNum, date: dateStr, focus: 'Rest & Review', subject: 'All', tasks: ['Light review of the week\'s topics', 'No new material today', 'Get good sleep'], rest: true })
          continue
        }

        const subject   = isLastWeek ? prioritized[d % prioritized.length] : subjects[d % subjects.length]
      ...
      weeks.push({ week: w + 1, theme: themes[w] || `Week ${w + 1}`, days: weekDays })
    }
    ...
    <div className="min-h-screen bg-white dark:bg-gray-950">
      <nav className="sticky top-0 z-50 bg-white/95 dark:bg-gray-950/95 backdrop-blur-sm border-b border-gray-100 dark:border-gray-800">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
          <Link href="/dashboard" className="text-sm text-gray-500 hover:text-gray-900 dark:hover:text-white">← Dashboard</Link>
          <span className="font-black">MyCET<span className="text-red-600">Reviewer</span></span>
          <div/>
        </div>
      </nav>
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        <div className="text-4xl mb-4">📅</div>
        <h1 className="text-2xl font-black text-gray-900 dark:text-white mb-3">30-Day Study Plan — Full Access Only</h1>
        <p className="text-sm text-gray-500 mb-8 max-w-md mx-auto">Get a personalized AI study plan based on your diagnostic results and target exam date.</p>
    ...
    <div className="min-h-screen bg-white dark:bg-gray-950">
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500">Personalized based on your diagnostic results and target exam.</p>
        </div>
          ...
          <div className="space-y-5">
            ...
            {weakTopics.length > 0 && (
              <div className="bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-2xl p-4">
                <div className="text-xs font-bold text-red-700 dark:text-red-400 uppercase tracking-wide mb-2">
                  Based on your diagnostic — weak topics to focus on
                </div>
            ...
            <div>
              <label className="block text-sm font-bold text-gray-900 dark:text-white mb-2">
                Exam date <span className="text-gray-400 font-normal">(optional — for countdown)</span>
              </label>
            ...
            {error && <div className="text-sm text-red-600 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-xl px-4 py-3">{error}</div>}

            <button onClick={generatePlan} disabled={generating}
              className="w-full bg-red-600 hover:bg-red-700 disabled:bg-gray-300 text-white font-black py-4 rounded-2xl transition-colors text-base">
              {generating ? 'Generating your plan...' : 'Generate My 30-Day Plan ✦'}
            </button>
            <p className="text-xs text-gray-400 text-center">AI-personalized based on your weak topics · Takes ~10 seconds</p>
          </div>
        ) : (
          <div className="space-y-4">
            {/* Plan header */}
            <div className="bg-gray-900 dark:bg-gray-800 rounded-2xl p-5 text-center">
            ...
            {plan.map(week => (
              <div key={week.week} className="border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
                <button onClick={() => setExpandedWeek(expandedWeek === week.week ? 0 : week.week)}
                  ...
                  <div className="text-gray-400 text-lg">{expandedWeek === week.week ? '↑' : '↓'}</div>
                </button>

                {expandedWeek === week.week && (
                  <div className="divide-y divide-gray-100 dark:divide-gray-700">
                    {week.days.map(day => (
                      <div key={day.day} className={`px-5 py-4 ${day.rest ? 'bg-green-50 dark:bg-green-950/30' : ''}`}>
                        <div className="flex items-start justify-between mb-2">
                          ...
                          {!day.rest && (
                            <Link href={`/training?topic=${encodeURIComponent(day.focus)}&school=${targetExam}`}
                              className="text-xs font-bold text-red-600 hover:text-red-800 bg-red-50 dark:bg-red-950 hover:bg-red-100 px-2.5 py-1 rounded-lg transition-colors flex-shrink-0 ml-3">
                              Train →
                            </Link>
                          )}
                        </div>
