'use client'
// src/app/results/page.tsx — v2
// Mock exam: shows ALL questions with correct/wrong + explanations after final section
// Free diagnostic: soft gate with email
// Paid diagnostic/training: full results immediately
// "Review wrong answers" section at bottom for mock
...
function aiCoach(pct: number, worst: string, xp: number, avgTime: number): string {
  ...
  if (pct >= 80) return `Strong result — ${pct}%. You earned <strong>+${xp} XP</strong>. Your weakest area is <strong>${worst}</strong>. Daily practice here will push your score higher.${timeNote}`
  if (pct >= 65) return `Solid — ${pct}%. You earned <strong>+${xp} XP</strong>. Focus on <strong>${worst}</strong>. Consistent daily sessions compound quickly.${timeNote}`
  if (pct >= 50) return `You scored ${pct}%. You earned <strong>+${xp} XP</strong>. Start with <strong>${worst}</strong> — it will have the highest impact on your score.${timeNote}`
  return `You scored ${pct}% — this is your starting point. You earned <strong>+${xp} XP</strong>. Build from <strong>${worst}</strong> and train daily. Improvement is measurable within two weeks.${timeNote}`
}
...
function validateEmail(email: string): string|null {
  ...
  if(fakeDomains.includes(domain)) return 'Please enter your real email address.'
  return null
}
...
export default function ResultsPage() {
  ...
  useEffect(()=>{
    ...
    // Paid users and non-diagnostic exams see full results immediately
    const userId = sessionStorage.getItem('userId')||localStorage.getItem('userId')
    if(examType==='mock'||examType==='training'){
      setUnlocked(true)
    }
    if(userId){
  ...
  let risk='High risk — focused training needed', riskCls='bg-red-50 text-red-700 border-red-200'
  if(pct>=70){risk='On track — keep training consistently';riskCls='bg-green-50 text-green-700 border-green-200'}
  else if(pct>=60){risk='Borderline — consistent daily training needed';riskCls='bg-yellow-50 text-yellow-700 border-yellow-200'}

  const examTypeLabel = examType==='mock'?'Mock Exam':examType==='training'?'Training':'Diagnostic'
  ...
  async function handleUnlock(e:React.FormEvent){
    ...
    try{
      const res=await fetch('/api/auth/register',{
        ...
        body:JSON.stringify({email:email.trim().toLowerCase(),school,results}),
      })
      if(res.ok){setUnlocked(true);sessionStorage.setItem('userEmail',email.trim().toLowerCase())}
      ...
      <div className="bg-gray-900">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 pt-8 pb-6">
          <div className="text-xs text-gray-400 font-semibold uppercase tracking-wider mb-1">
            {EXAM_NAMES[school]} {examTypeLabel} Results
          </div>
        ...
        <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5 flex items-center gap-5">
          ...
          <div>
            <div className="font-bold text-base text-gray-900 mb-1">
              {pct>=70?`${EXAM_NAMES[school]} Ready 🎉`:pct>=55?'Almost there!':'More training needed'}
            </div>
              ...
              <div className="bg-gray-900 rounded-2xl p-5">
                ...
                <form onSubmit={handleUnlock} className="space-y-3">
                  ...
                  <button type="submit" disabled={loading}
                    className="w-full bg-red-600 hover:bg-red-700 disabled:bg-gray-600 text-white font-bold py-3 rounded-xl text-sm transition-colors">
          ...
          <>
            <div className="bg-gray-50 border border-gray-200 rounded-2xl p-5">
              ...
              <div className="text-sm text-gray-700 leading-relaxed"
                dangerouslySetInnerHTML={{__html:aiCoach(pct,weakTopics[0]?.split('—')[1]?.trim()||weakTopics[0]||'weak topics',results.examXP,avgTime)}}/>
            </div>
            ...
            {/* Review wrong answers — mock/training only */}
            {examType !== 'diagnostic' && wrongAnswers.length > 0 && (
              <div>
            ...
            <div className="space-y-3 pb-6">
              {examType==='mock' ? (
                <>
                  ...
                  <Link href="/mock"
                    className="block w-full text-center border border-gray-300 hover:bg-gray-50 text-gray-700 font-semibold py-3.5 rounded-2xl transition-colors text-sm">
                    Back to Mock Exams
                  </Link>
                </>
              ) : examType==='training' ? (
