...
// A passage item has correct_index === -1 AND no choices (empty array or undefined)
function isPassageItem(q: any): boolean {
  const noChoices = !q.choices || q.choices.length === 0
  const negIdx    = q.correct_index === -1 || q.correct === -1
  return negIdx && noChoices
}

function getPassageText(q: any): string {
  return (q.question || q.passage || '').toString().replace(/^PASSAGE:\s*/i, '').trim()
}

function normalizeQuestion(q: any) {
  const passage = isPassageItem(q)
  return {
    ...q,
...
const SEED: any[] = [
  ...
  { id:'u2',  subject:'Mathematics',           topic:'Statistics',          difficulty:'hard',   question:'Mean of 5 numbers is 12. One removed, new mean is 14. Removed?', choices:['4','2','6','8'],                                                                                                                                      correct:0, explanation:'Sum=60. New sum=56. Removed=4.' },
  { id:'u3',  subject:'Language Proficiency',  topic:'Grammar',             difficulty:'hard',   question:'Which sentence is CORRECT?',                                     choices:['Neither the captain nor the players was ready.','Neither the captain nor the players were ready.','Neither the captain nor the players are not ready.','Neither and the players were ready.'], correct:1, explanation:"'Players' is closer → 'were'." },
  { id:'u4',  subject:'Reading Comprehension', topic:'Inference',           difficulty:'hard',   question:"'Researchers who spent decades defending a theory fiercely opposed the young scientist.' Inferred:", choices:['They protected their reputation.','They had not read her paper.','They found errors.','They were envious.'],  correct:0, explanation:"'Decades defending' = investment in position." },
  ...
  { id:'u6',  subject:'Science',               topic:'Physics',             difficulty:'easy',   question:'5 kg object pushed with 20 N. Acceleration?',                   choices:['4 m/s²','100 m/s²','0.25 m/s²','2 m/s²'],                                                                                                           correct:0, explanation:'F=ma → a=20÷5=4 m/s².' },
  { id:'u7',  subject:'Mathematics',           topic:'Geometry',            difficulty:'medium', question:'Triangle has sides 5, 12, 13. What is its area?',               choices:['30 cm²','60 cm²','65 cm²','32.5 cm²'],                                                                                                               correct:0, explanation:'Right triangle. Area=½×5×12=30 cm².' },
  ...
  { id:'u10', subject:'Reading Comprehension', topic:'Tone',                difficulty:'medium', question:"'Industry leaders greeted the policy with cautious optimism.' Tone:", choices:['Enthusiastically positive','Neutral to mildly positive','Critical','Ironic'],                                                                    correct:1, explanation:"'Cautious optimism' = qualified positivity." },
  { id:'u11', subject:'Mathematics',           topic:'Patterns',            difficulty:'medium', question:'Next in sequence: 2, 6, 12, 20, 30, ___',                       choices:['42','40','36','44'],                                                                                                                                  correct:0, explanation:'Differences: +4,+6,+8,+10,+12. Next: 30+12=42.' },
  ...
  { id:'u13', subject:'Language Proficiency',  topic:'Verb Tense',          difficulty:'hard',   question:"Complete: 'By the time she arrives, I _____ dinner.'",           choices:['finished','will have finished','have finished','was finishing'],                                                                                      correct:1, explanation:"Future perfect: 'will have finished'." },
  { id:'u14', subject:'Science',               topic:'Ecology',             difficulty:'medium', question:'Grass→rabbit→fox→wolf. Wolves disappear. What happens FIRST?',   choices:['Grass increases','Fox population increases','Rabbit decreases','Grass decreases'],                                                                    correct:1, explanation:'Remove wolves → fox population rises first.' },
  { id:'u15', subject:'Mathematics',           topic:'Word Problems',       difficulty:'hard',   question:'Manila→Batangas 2hrs at 90kph. Return at 60kph. Average speed?', choices:['75 kph','72 kph','68 kph','80 kph'],                                                                                                                  correct:1, explanation:'Harmonic mean: 2÷(1/90+1/60)=72 kph.' },
  ...
  { id:'u20', subject:'Science',               topic:'Physics',             difficulty:'medium', question:"Newton's 3rd Law: when you push a wall, the wall:",              choices:['Does nothing','Pushes back with equal force','Pushes back with less force','Absorbs the force'],                                                     correct:1, explanation:'Equal and opposite reaction.' },
]

const XP: Record<string,number>    = { easy:10, medium:20, hard:30 }
const TIMER: Record<string,number> = { easy:30, medium:45, hard:60 }
const SCHOOL: Record<string,string> = { upcat:'UPCAT', acet:'ACET', dcat:'DCAT', ustet:'USTET', pupcet:'PUPCET', suc:'State U CET' }
...
export default function ExamEngine() {
  ...
  const realQs     = queue.filter(q => !q.isPassage)
  ...
  const prevPassage = !isPassage ? queue.slice(0,idx).reverse().find(q => q.isPassage) : null
  ...
  function handleAnswer(chosenIdx: number) {
    ...
    setAnswers(prev => [...prev, {questionId:currentQ.id, question:currentQ, chosen:chosenIdx, isCorrect:ok, timeSeconds:t, xpEarned:xp}])
    if (!isMock) showToast(ok ? `Correct! +${xp} XP` : 'Incorrect — check the explanation.')
  }

  async function handleNext() {
    if (isPassage) { setIdx(prev => prev+1); setChosen(null); setRevealed(false); return }
    if (!revealed) return
    const next = idx+1
      ...
      if (userId) {
        try {
          await fetch('/api/exam/save', {
            ...
            body: JSON.stringify({userId, school, examType, sessionId, answers:finalAnswers, examXP, totalQuestions:realQs.length, srCount:0})
          })
        } catch {}
      }
      sessionStorage.setItem('examResults', JSON.stringify({school, examType, answers:finalAnswers, examXP, totalQuestions:realQs.length, srCount:0}))
      setSaving(false)
      ...
      <div className="min-h-screen bg-white">
        <div className="bg-gray-900 sticky top-0 z-40">
          <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
            <div>
              <div className="text-xs text-gray-400 uppercase tracking-wide">{SCHOOL[school]} · {examType} · Reading</div>
              <div className="text-xs text-gray-500 mt-0.5">Read carefully — questions follow</div>
            </div>
            <div className="bg-blue-800 text-blue-200 text-xs font-bold px-2 py-1 rounded-full">READ PASSAGE</div>
          </div>
          <div className="h-1 bg-gray-800"><div className="h-1 bg-red-600" style={{width:`${progress}%`}}/></div>
        </div>

        <div className="max-w-2xl mx-auto px-4 py-8">
          <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6 mb-6">
            <div className="text-xs font-bold text-blue-600 uppercase tracking-wide mb-3">Read this passage carefully</div>
            <p className="text-base text-gray-800 leading-relaxed">{getPassageText(currentQ)}</p>
          </div>
          <button onClick={handleNext}
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-2xl transition-colors">
            I have read this passage — Continue to Questions →
          </button>
          <p className="text-xs text-gray-400 text-center mt-3">The passage will appear above each question for reference.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-gray-900 sticky top-0 z-40">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          <div>
            <div className="text-xs text-gray-400 uppercase tracking-wide">{SCHOOL[school]} · {examType} · {currentQ.subject}</div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        {/* Passage reference */}
        {prevPassage && (
          <details className="mb-5 bg-blue-50 border border-blue-200 rounded-xl">
            <summary className="px-4 py-3 text-xs font-bold text-blue-700 cursor-pointer list-none flex items-center justify-between">
              <span>📖 View passage</span><span className="text-gray-400 font-normal">tap to expand</span>
            </summary>
            <div className="px-4 pb-4 pt-2 text-sm text-gray-700 leading-relaxed border-t border-blue-200">
              {getPassageText(prevPassage)}
            </div>
          </details>
        )}

        <div className="flex items-center gap-2 mb-5">
          <span className="text-sm text-gray-400">Q{realQIdx} of {realQs.length}</span>
